...
struct ScatterRecord {
  Color attenuation_;
  SPtr<PDF> next_ray_samplling_pdf_ptr_;
  bool skip_pdf_;
  Ray skip_pdf_ray_;
};
class Material {
...
// public:
//   bool scatter(const Ray &ray_in, const HitRecord &rec,ScatterRecord
//   &scatter_record) const override {
...
public:
  ...
  bool scatter(const Ray &r_in, const HitRecord &rec,
    ...
    scatter_record.next_ray_samplling_pdf_ptr_ =
        std::make_shared<CosineHemispherePDF>(rec.normal_);
...
public:
  ...
  bool scatter(const Ray &r_in, const HitRecord &rec,
               ScatterRecord &scatter_record) const override {
    scatter_record.attenuation_ = albedo_;
    scatter_record.next_ray_samplling_pdf_ptr_ = nullptr;
    scatter_record.skip_pdf_ = true;
    Vec3 reflected = reflect(unit_vector(r_in.direction()), rec.normal_);
    scatter_record.skip_pdf_ray_ =
        Ray{rec.p_, reflected + fuzz_ * random_in_unit_sphere(), r_in.time()};
    ...
    // attenuation = albedo_;
    // return (dot(scattered.direction(), rec.normal_) > 0); // must in same
    // side return true;
  }
...
public:
  ...
  bool scatter(const Ray &ray_in, const HitRecord &rec,
    ...
    scatter_record.attenuation_ = albedo_->value(rec.u_, rec.v_, rec.p_);
    scatter_record.next_ray_samplling_pdf_ptr_ =
        std::make_shared<UniformSpherePDF>();
    scatter_record.skip_pdf_ = false;
    return true;
  }
  double scattering_pdf(const Ray &r_in, const HitRecord &rec,
