...
//     2*pi*sin(θ)*f(θ)
class UniformSamplingOnHemiSphere {
 public:
  UniformSamplingOnHemiSphere() : rd_{}, gen_{rd_()}, d_{0.0} {}
...
int main() {
  UniformSamplingOnHemiSphere random_in_unit_hemi_sphere;
  std::vector<double> xs;
  std::vector<double> ys;
  std::vector<double> zs;
  int smaple_num = 100000;
  for (int i = 0; i < smaple_num; i++) {
    auto [x, y, z] = random_in_unit_hemi_sphere.sample();
    xs.push_back(x);
    ys.push_back(y);
    zs.push_back(z);
  }
