  ...
  for (; depth < max_depth; depth++) {
    ...
    if (world.hit(cur_ray, Interval{0.001, INFINITY_}, rec)) {
      ...
      if (rec.mat_->scatter(cur_ray, rec, attenuation,
        ...
        Vec3 to_light{};
        // sampling direction to light
        auto on_light =
            Point3{random_double(213, 343), 554, random_double(227, 332)};
        to_light = on_light - rec.p_;
        auto distance_squared = to_light.length_squared();
        to_light = unit_vector(to_light);
        // oppsite
        if (dot(to_light, rec.normal_) < 0) {
        ...
        auto nex_ray_sampling_pdf =
            distance_squared / (light_cosine * light_area);
        //overwriten scattered
        scattered = Ray{rec.p_, to_light, ray.time()}; // directly go to light
