...
int main() {
  try {
    ...
    camera.vfov_ = 20;
    camera.lookfrom_ = Point3{26, 3, 6};
    camera.lookat_ = Point3{0, 2, 0};
    ...
    int thread_num = 5;
    {
      ...
      world.add(make_static_sphere(Point3{0, 2, 0}, 2,
                                   make_lambertian_material(pertext)));
      auto difflight = make_diffuse_light_material(Color{4, 4, 4});
      world.add(make_static_sphere(Point3{0,7,0}, 2, difflight));
      world.add(
          make_quad(Point3{3, 1, -2}, Vec3{2, 0, 0}, Vec3{0, 2, 0}, difflight));
