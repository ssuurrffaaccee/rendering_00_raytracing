  ...
  for (; depth < max_depth; depth++) {
    ...
    if (world.hit(cur_ray, Interval{0.001, INFINITY_}, rec)) {
      ...
      if (rec.mat_->scatter(cur_ray, rec, attenuation,
        ...
        auto p0 = std::make_shared<HittablePDF>(lights, rec.p_);
        auto p1 = std::make_shared<CosineHemispherePDF>(rec.normal_);
        MixturePDF mixed_pdf{p0, p1};
        // HittablePDF light_pdf{lights, rec.p_};
        Vec3 scatter_direction = mixed_pdf.sample();
        // overwriten scattered
        scattered = Ray{rec.p_, scatter_direction, ray.time()};
        ...
        // auto nex_ray_sampling_pdf =
        //     rec.mat_->next_ray_sampling_pdf(ray, rec, scattered);
        final_color += acc_attenuation_times_scattering_over_sampling_pdf *
                       color_from_emission;
        acc_attenuation_times_scattering_over_sampling_pdf =
            acc_attenuation_times_scattering_over_sampling_pdf *
            (attenuation * scattering_pdf / nex_ray_sampling_pdf);
        cur_ray = scattered;
        continue;
...
int main() {
  try {
    ...
    {
      HittableList world;
      auto [world_ptr, lights_ptr] = make_cornell_box_with_explicit_lights();
