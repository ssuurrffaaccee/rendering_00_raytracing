...
int main() {
  try {
    ...
    camera.vfov_ = 20;
    camera.lookfrom_ = Point3{-2, 2, 1};
    camera.lookat_ = Point3{0, 0, -1};
    camera.vup_ = Vec3{0, 1, 0};
    camera.defocus_angle_ = 10.0;
    camera.focus_dist_ = 3.4;
    ...
    auto material_center = make_lambertian_material(Color(0.1, 0.2, 0.5));
    ...
    world.add(
        make_sphere(Point3{0.0, 0.0, -1.0}, -0.5,
                    material_left));  // why? -> only radius*raduis is used for
                                      // calculating intersection.
    world.add(make_sphere(Point3{-1.0, 0.0, -1.0}, 0.5, material_right));
    world.add(make_sphere(Point3{1.0, 0.0, -1.0}, 0.5, material_center));
