...
int main() {
  try {
    ...
    camera.vfov_ = 20;
    camera.lookfrom_ = Point3{-5, 10, -12};
    ...
    HittableList world;
    {
      auto earth_texture = make_image_texture("../../resources/earthmap.jpg");
      // auto earth_texture =
      //     make_uv_view_texture(4, Color{0.0, 0.0, 0.0},
      //     Color{1.0, 1.0, 1.0});
      auto earth_surface = make_lambertian_material(earth_texture);
      auto globe = make_static_sphere(Point3{0, 0, 0}, 2, earth_surface);
      world.add(globe);
    }
